...
namespace GNet.IO
{
    ...
    {
        static InputSimulator current;
        public static InputSimulator Current
        {
            get
            {
                if (current == null)
                    current = new InputSimulator();
        ...
        public uint KeyDown(ScanCode scanCode, bool extended = false)
        {
            uint retval = DoKeyDown(scanCode, extended);
